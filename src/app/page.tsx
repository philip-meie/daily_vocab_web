...
export default function Home() {
  ...
  const handleSubmitSentence = async () => {
    ...
    if (!res.ok) {
      console.log("Error validating sentence");
      return;
    }
  ...
  return (
    <div className="container mx-auto p-4 max-w-3xl">
      <h1 className="text-4xl md:text-5xl font-extrabold text-center mb-8 text-gray-800 leading-tight">
        Word Challenge
      </h1>
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl mb-6 border border-gray-100 transform hover:scale-105 transition-transform duration-300 ease-in-out">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
          <h2 className="text-3xl md:text-4xl font-bold text-primary mb-2 sm:mb-0">
            {currentWord.word}
          </h2>
          <span
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 space-y-4 sm:space-y-0">
          <p className="text-2xl font-bold">
            Score:{' '}
            <span className={`${feedbackColor} transition-colors duration-300`}>
              {score.toFixed(1)}
            </span>
          </p>
